...
import WomenPersonaTestPage from './features/persona/pages/WomenPersonaTestPage';
import WomenPersonaResultPage from './features/persona/pages/WomenPersonaResultPage';
...
function App() {
  ...
  // [추가] 남성/여성 검사 흐름 구분용 상태
  const [selectedGender, setSelectedGender] = useState("men");
  const [womenPersonaResult, setWomenPersonaResult] = useState(null);
  ...
  // PurchasePage로 넘겨줄 최종 선택 아이템 상태
  ...
  const handleAnswer = (answer) => {
    ...
    if (step === 'step1') {
      const newScores = {
        ...typeScores,
        [answer.type]: typeScores[answer.type] + 1
      };
      ...
      const newPersonaScores = {
        ...personaScores,
        [targetPersona]: (personaScores[targetPersona] || 0) + 1
      };
      ...
      if (currentIdx + 1 < currentGroupQuestions.length) {
        ...
        const finalResult = Object.keys(newPersonaScores).reduce((a, b) =>
          newPersonaScores[a] >= newPersonaScores[b] ? a : b
        );
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      // 현재 백엔드(app.py)를 수정하지 않아도 동작하도록 persona 중심으로 요청합니다.
      // 나중에 여성복 DB/추천 로직을 분리하면 gender도 백엔드에서 받아 처리하면 됩니다.
      const queryParams = new URLSearchParams({
        persona: result
      });

      // 참고: 백엔드에서 gender를 받을 준비가 되면 아래 줄을 활성화해도 됩니다.
  ...
  return (
    <div className="App">
      ...
      {step === "women_test" && (
        <WomenPersonaTestPage
          onBack={() => setStep("main")}
          onComplete={(womenResult) => {
            setWomenPersonaResult(womenResult);
            setResult(womenResult.persona);
            setSelectedGender("women");
            setStep("women_result");
          }}
        />
      )}

      {step === "women_result" && (
        <WomenPersonaResultPage
          result={womenPersonaResult}
          onRestart={() => {
            setWomenPersonaResult(null);
            setResult("");
            setStep("women_test");
          ...
          onBackHome={resetAll}
          onGoToCollage={() => setStep("price_setting")}
        />
      )}
        ...
        <div className="question-container fade-in">
          <div className="progress-bar">
            <div
              className="progress"
              style={{
                width: `${((step === 'step1' ? currentIdx : currentIdx + 4) / 8) * 100}%`
              }}
            />
          </div>

          <p className="q-count">
            Q. {step === 'step1' ? currentIdx + 1 : currentIdx + 5} / 8
          </p>

          <h2 className="question-text">
            {step === 'step1'
              ? step1Questions[currentIdx].q
              : step2Groups[selectedType].questions[currentIdx].q}
          </h2>

          <div className="answer-group">
            {(step === 'step1'
              ? step1Questions[currentIdx].a
              : step2Groups[selectedType].questions[currentIdx].a
            ).map((a, i) => (
              <button
                key={i}
                className="ans-btn"
